...
export default function ManagerSidebar({
  ...
  return (
    <aside
      className={[
        'sidebar-filter fixed left-0 top-0 z-40 h-screen w-[280px] bg-[#f37021] text-white flex flex-col transition-transform duration-300 ease-out',
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto py-3 px-3 no-scrollbar">
        {MANAGER_MENU.map((group) => (
          <div key={group.group} className="mb-3">
            {/* Group label */}
            <p className="text-xs font-semibold tracking-widest text-white uppercase mb-1.5 px-3">
              {group.group}
            </p>
      ...
      {/* Logout */}
      <div className="p-4 border-t border-[#d4621c] shrink-0">
        <button
          type="button"
          onClick={() => {
            logout();
